...
| `sample_result.png` | "BATTLE COMPLETE", large score "24", caption "POINTS · 02:00 · TIME'S UP", PLAY AGAIN / MAIN MENU | **Record status** and a retry action (UI-04); a variant for defeat |
...
| `sample_history.png` | Same panel, MATCH HISTORY active. Caption "CAPTAIN JACK · YOUR RECENT BATTLES". Columns DATE / POINTS / DURATION / RESULT. Result is "TIME'S UP" (green tint) or "DEFEATED" (red tint). Newest row highlighted. "PAGE 1 OF 2" | Loading, empty, error states; pending (unconfirmed) records |
